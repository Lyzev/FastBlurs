...
#define DEBUG_PRINT(s, ...) {TCHAR sOut[2048]; swprintf_s(sOut, 2048, s, __VA_ARGS__); OutputDebugString(sOut); OutputDebugString(L"\n");}
#define DEBUG_PRINTA(s, ...) {CHAR sOut[2048]; sprintf_s(sOut, 2048, s, __VA_ARGS__); OutputDebugStringA(sOut); OutputDebugString("\n");}
#else 
#define DEBUG_PRINT(...)
#endif
#endif
...
public:
    ...
    virtual CPUTEventHandledCode CPUTHandleMouseEvent(int x, int y, int wheel, CPUTMouseState state, CPUTEventID eventID)
                {UNREFERENCED_PARAMETER(x);UNREFERENCED_PARAMETER(y);UNREFERENCED_PARAMETER(wheel);UNREFERENCED_PARAMETER(state);UNREFERENCED_PARAMETER(eventID);return CPUT_EVENT_UNHANDLED;}
