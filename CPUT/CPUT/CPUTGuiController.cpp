...
//-----------------------------------------------------------------------------
CPUTEventHandledCode CPUTGuiController::HandleMouseEvent(int x, int y, int wheel, CPUTMouseState state, CPUTEventID eventID)
{
    ...
    for(UINT i=0; i<mControlPanelIDList[mActiveControlPanelSlotID]->mControlList.size(); i++)
    {
        if(CPUT_EVENT_HANDLED == EventResult)
        {
            ...
            mControlPanelIDList[mActiveControlPanelSlotID]->mControlList[i]->HandleMouseEvent(-1,-1,wheel,state, eventID);
        }
        else
        {
            //mControlPanelIDList[mActiveControlPanelSlotID]->mControlList[i]->GetControlID();
            CPUTControlType type = mControlPanelIDList[mActiveControlPanelSlotID]->mControlList[i]->GetType();

            EventResult = mControlPanelIDList[mActiveControlPanelSlotID]->mControlList[i]->HandleMouseEvent(x,y,wheel,state, eventID);
