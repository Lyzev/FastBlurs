...
CPUTEventHandledCode CPUTDropdown::HandleMouseEvent(int x, int y, int wheel, CPUTMouseState state, CPUTEventID eventID)
{
    ...
    if(trayOpen)
        DEBUG_PRINT(_L("TRAY OPEN"));
    if(mouseInTop)
        DEBUG_PRINT(_L("MOUSE IN TOP"));
    if(mouseInTray)
        DEBUG_PRINT(_L("MOUSE IN TRAY"));

    if(trayOpen == false)
    {
        if(eventID == CPUT_EVENT_DOWN && state & CPUT_MOUSE_LEFT_DOWN && mouseInTop)
        {
            DEBUG_PRINT(_L("OPEN TRAY"));
