...
)
{
    ...
    *ppPixelShader = CPUTShaderOGL::CreateShaderFromFiles( finalNames, GL_FRAGMENT_SHADER, CPUT_OGL::GetGLSL_VERSION(), CPUT_OGL::DEFAULT_MACROS, pShaderMacros);
...
)
{
    ...
    *ppVertexShader = CPUTShaderOGL::CreateShaderFromFiles( finalNames, GL_VERTEX_SHADER, CPUT_OGL::GetGLSL_VERSION(), CPUT_OGL::DEFAULT_MACROS, pShaderMacros);
...
)
{
    ...
    *ppHullShader = CPUTShaderOGL::CreateShaderFromFiles( finalNames, GL_TESS_CONTROL_SHADER, CPUT_OGL::GetGLSL_VERSION(), CPUT_OGL::DEFAULT_MACROS, pShaderMacros);
...
)
{
    ...
    *ppDomainShader = CPUTShaderOGL::CreateShaderFromFiles( finalNames, GL_TESS_EVALUATION_SHADER, CPUT_OGL::GetGLSL_VERSION(), CPUT_OGL::DEFAULT_MACROS, pShaderMacros);
...
)
{
    ...
    *ppGeometryShader = CPUTShaderOGL::CreateShaderFromFiles( finalNames, GL_GEOMETRY_SHADER, CPUT_OGL::GetGLSL_VERSION(), CPUT_OGL::DEFAULT_MACROS, pShaderMacros);

    return result;
}

CPUTResult CPUTAssetLibraryOGL::GetComputeShader(
    const cString       &name,
    const cString       &shaderMain,
    const cString       &shaderProfile,
    CPUTShaderOGL       **ppComputeShader,
    bool                nameIsFullPathAndFilename,
    CPUT_SHADER_MACRO   *pShaderMacros
)
{
    std::vector<cString> filenames;
    filenames.push_back(name);
    return GetComputeShader(filenames, shaderMain, shaderProfile, ppComputeShader, nameIsFullPathAndFilename, pShaderMacros);
}
...
)
{
    ...
    // see if the shader is already in the library
    void *pShader = FindComputeShader(libName + shaderMain + shaderProfile, true);
    if(NULL!=pShader)
    {
        *ppComputeShader = (CPUTShaderOGL*)pShader;
        (*ppComputeShader)->AddRef();
        return result;
    }
    *ppComputeShader = CPUTShaderOGL::CreateShaderFromFiles( finalNames, GL_COMPUTE_SHADER, CPUT_OGL::GetGLSL_VERSION(), CPUT_OGL::DEFAULT_MACROS, pShaderMacros);
