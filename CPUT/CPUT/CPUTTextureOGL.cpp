...
// Why on earth are you redefining things here. Pull in the correct values!
...
static GLuint LoadTexturePNG( const cString TextureFileName, bool forceSRGB = true, int * pOutWidth = NULL, int * pOutHeight = NULL )
{
    // Load Texture File
    char* pFileData = NULL;
    unsigned int fileSize = 0;
    ...
    {
        DEBUG_PRINT( _L("Success loading texture file %s"), TextureFileName.c_str() );
    }
    else
    {
        DEBUG_PRINT( _L("Error loading texture file %s"), TextureFileName.c_str() );
    }
    ...
    if( pOutWidth != NULL )     *pOutWidth  = width;
    if( pOutHeight != NULL )    *pOutHeight = height;
...
#define BOARDER 0
    //glTexStorage2D instead of glTexImage2D - see the email. add flag here for that. stupid opengl
    if( pData != NULL )
    {
...
//    ID3D11Device *pD3dDevice= CPUT_DX11::GetDevice();
    CPUTResult result = CreateNativeTexture( /*pD3dDevice, */absolutePathAndFilename, /*&pShaderResourceView,*/ &textureID, loadAsSRGB, &width, &height );
    ...
    pNewTexture->mWidth     = width;
    pNewTexture->mHeight    = height;
#else
    width; height;
#endif
...
//    ID3D11Resource **ppTexture,
    GLint *textureID,
    bool ForceLoadAsSRGB, int * pOutWidth, int * pOutHeight )
{
    GLint textureName = 0;
    ...
    if (!ext.compare(_L("dds")))
    {
        ...
        textureName = LoadTexturePNG( fileName, ForceLoadAsSRGB, &width, &height );
