...
//    mD3DMeshTopology(0),
    ...
    mpIndexBuffer(NULL),
    mIndexCount(0),
    mIndexBufferDynamic(false),
    mVertexCount(0),
    mVertexStride(0),
    mpVertexArray(NULL),
    mVertexBufferDynamic(false)
{
}
...
CPUTResult CPUTMeshOGL::CreateNativeResources(
    ...
    UINT                    indexCount,
    void                   *pIndexData,
    bool                    indexBufferDynamic
)
{
    mpVertexArray = new CPUTVertexArrayOGL(vertexElementCount);
	mIndexCount = indexCount;
	mVertexCount = vertexCount;
    mIndexBufferDynamic = indexBufferDynamic;
    mVertexBufferDynamic = vertexBufferDynamic;

	SAFE_RELEASE(mpIndexBuffer);
	if(indexCount > 0)
	{
		mpIndexBuffer = new CPUTBufferOGL(_L("index buffer"), GL_ELEMENT_ARRAY_BUFFER, (indexBufferDynamic)?(GL_DYNAMIC_DRAW):(GL_STATIC_DRAW), mIndexCount * pIndexDataInfo->mElementSizeInBytes, NULL);
		if(pIndexData != NULL)
	...
	if(vertexCount > 0)
	{
		...
		mpVertexBuffer = new CPUTBufferOGL(_L("vertex buffer"), GL_ARRAY_BUFFER, (vertexBufferDynamic)?(GL_DYNAMIC_DRAW):(GL_STATIC_DRAW), mVertexCount * mVertexStride, NULL);
        if(pVertexData != NULL)
...
{
    assert( mIndexBufferDynamic );
    if( !mIndexBufferDynamic )
        return;
    
    mpVertexArray->Enable(); // for some reason needed on Nexus7
    mpIndexBuffer->SetSubData(offset, size, pData);
    mpVertexArray->Disable();  // for some reason needed on Nexus7
}
void CPUTMeshOGL::SetVertexSubData( UINT offset, UINT size, void* pData)
{
    assert( mVertexBufferDynamic );
    if( !mVertexBufferDynamic )
        return;
    mpVertexArray->Enable(); // for some reason needed on Nexus7
