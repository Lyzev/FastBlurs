...
void CPUTGuiControllerOGL::Draw()
{
    ...
    if(mUberBufferDirty)
    {
        ...
        // Prevent looping refresh as some of the above code triggers unwanted resize requests which then perpetuate forever
        for(int ii=0; ii<ControlCount; ii++)
        {
            CPUTControl *pControl = mControlPanelIDList[mActiveControlPanelSlotID]->mControlList[ii];
            if(true == pControl->ControlResizedItself())
...
#warning "Need to do something with uniform buffers here"
    ...
    mpControlMaterial->SetRenderStates(params);
    ...
    mpTextMaterial->SetRenderStates(params);
...
#warning "Need to do something with uniform buffers here"
    ...
    mpTextMaterial = (CPUTMaterialOGL*)pAssetLibrary->GetMaterial(_L("guimaterial_text"));
    mpControlMaterial = (CPUTMaterialOGL*)pAssetLibrary->GetMaterial(_L("guimaterial_control"));
    ...
    mpFont = CPUTFontOGL::CreateFont(_L("HUH"), _L(""));    
    ...
    mpUberBuffer->CreateNativeResources(NULL, 0, 3, pGUIVertex, CPUT_GUI_VERTEX_BUFFER_SIZE, mpMirrorBuffer, true, NULL, 0, NULL, false);

    mpTextUberBuffer = new CPUTMeshOGL();
    mpTextUberBuffer->CreateNativeResources(NULL, 0, 3, pGUIVertex, CPUT_GUI_VERTEX_BUFFER_SIZE, NULL, true, NULL, 0, NULL, false);

    mpFocusedControlBuffer = new CPUTMeshOGL();
	mpFocusedControlBuffer->CreateNativeResources(NULL, 0, 3, pGUIVertex, CPUT_GUI_VERTEX_BUFFER_SIZE, mpFocusedControlMirrorBuffer, true, NULL, 0, NULL, false);

    mpFocusedControlTextBuffer = new CPUTMeshOGL();
	mpFocusedControlTextBuffer->CreateNativeResources(NULL, 0, 3, pGUIVertex, CPUT_GUI_VERTEX_BUFFER_SIZE, mpFocusedControlTextMirrorBuffer, true, NULL, 0, NULL, false);
