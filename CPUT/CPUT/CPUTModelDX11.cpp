...
//            CPUTMaterialDX11 *pOriginalMaterial = (CPUTMaterialDX11*)(mpRootMaterial[ii]->GetSubMaterials()[OriginalMaterialIndex]);
			...
			mDrawModelCallBackFunc(this, renderParams, mpMesh[ii],pMaterial,NULL,mpInputLayout[ii][finalMaterialIndex]);
...
bool CPUTModelDX11::DrawModelCallBack(CPUTModel*, CPUTRenderParameters &renderParams, CPUTMesh* pMesh, CPUTMaterial* pMaterial, CPUTMaterial* pOriginalMaterial, void* pInputLayout)
{
...
CPUTResult CPUTModelDX11::LoadModel(CPUTConfigBlock *pBlock, int *pParentID, CPUTModel *pMasterModel, int numSystemMaterials, cString *pSystemMaterialNames)
{
    ...
    *pParentID = pBlock->GetValueByName(_L("parent"))->ValueAsInt();
    ...
    for(UINT ii=0; ii<mMeshCount; ii++)
    {
        ...
        CPUTMaterial **pSubMaterials = pMaterial->GetSubMaterials();
        SetSubMaterials(ii, pSubMaterials);
