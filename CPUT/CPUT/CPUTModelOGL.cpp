...
void CPUTModelOGL::Render(CPUTRenderParameters &renderParams, int materialIndex)
{
    ...
    for(UINT ii=0; ii<mMeshCount; ii++)
    {
        ...
        CPUTMaterialOGL *pMaterial = (CPUTMaterialOGL*)(mpMaterial[ii][finalMaterialIndex]);

		// Needed if shadow shorting of transparent objects needed
  ...
  //      CPUTMaterialOGL *pOriginalMaterial = (CPUTMaterialOGL*)(mpRootMaterial[ii]->GetSubMaterials()[OriginalMaterialIndex]);
...
//-----------------------------------------------------------------------------
bool CPUTModelOGL::DrawModelCallBack(CPUTModel* pModel, CPUTRenderParameters &renderParams, CPUTMesh* pMesh, CPUTMaterial* pMaterial, CPUTMaterial* , void* )
{
  
	pMaterial->SetRenderStates(renderParams);
    if( ((CPUTMaterialOGL*)pMaterial)->Tessellated() )
...
//        pFinalSystemNames[totalNameCount + CPUT_MATERIAL_INDEX_BOUNDING_BOX]                = _L("%BoundingBox");
        ...
        CPUTMaterialOGL *pMaterial = (CPUTMaterialOGL*)pAssetLibrary->GetMaterial(materialName, false, this, ii, NULL, finalNumSystemMaterials, pFinalSystemNames);
...
//-----------------------------------------------------------------------------
void CPUTModelOGL::SetSubMaterials(UINT ii, CPUTMaterial **pMaterial)
{
